...
int main(int argc, char* argv[]) {
    if (argc != 3) {
        ...
        while ( 0 > recvfrom(client_socket, &ack_operation_packet, sizeof(operation_datagram_t), 0, (struct sockaddr*)&server_addr, &server_addrLen))
        {
            printf("Failed to receive ack from server, retrying connection...\n");
            printCurrentTime();
            check(
                (sendto(client_socket, &operation_packet, sizeof(operation_datagram_t), 0, (struct sockaddr*)&server_addr, sizeof(server_addr))),
                "Failed to send operation datagram.\n"
